...
# SSH credential fallback: the service account (cisco_svc) is tried first; if it
# is rejected (AUTH_FAIL), local accounts are tried in order, one at a time.
# Local accounts are auto-discovered from numbered sections in credentials.json/
# credentials.ini: [cisco_local], [cisco_local2], [cisco_local3], ... Any section
# missing a username or password is skipped. The report shows which account
# actually logged in.
...
def _load_local_accounts():
    """Auto-discover local-user fallback accounts from numbered credential
    sections (mirrors cisco_detect_device_os.py). Any section missing a
    ...
    for section in LOCAL_ACCOUNT_SECTIONS:
        ...
        if local_username and local_password:
            local_secret = (config.get_cred(section, 'secret')
                             or config.get_cred(section, 'enable')
                             or local_password)
            accounts.append({
                'label': section,
...
def connect_and_detect(host, creds):
    ...
    fallbacks like cisco_local/cisco_local2/...). Enters enable mode if we
    land in user EXEC, then detects platform + model.
...
def main():
    ...
    secret    = config.get_cred('cisco_svc', 'secret') or config.get_cred('cisco_default', 'enable')
    ...
    # (auto-discovered from numbered credential sections, same as
    # cisco_detect_device_os.py).
    creds = [(username, password, secret, "cisco_svc")]
    for acct in LOCAL_ACCOUNTS:
        creds.append((acct['username'], acct['password'], acct['secret'], acct['label']))

    if LOCAL_ACCOUNTS:
        names = ', '.join(acct['username'] for acct in LOCAL_ACCOUNTS)
        print(f"Local-user fallback is enabled for {len(LOCAL_ACCOUNTS)} account(s): {names}\n")
    else:
        print("Local-user fallback is not configured "
              "(add [cisco_local], [cisco_local2], ... to credentials.json/credentials.ini to enable it).\n")
